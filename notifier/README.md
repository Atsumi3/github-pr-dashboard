...
3. 新規 PR ごとに macOS 通知を表示（terminal-notifier があればクリックで `openUrl` を開く。無ければ osascript）
...
- `terminal-notifier`（任意・推奨）。あると通知クリックで `openUrl`（既定 http://localhost:3000）を開けます。無い場合は osascript で通知しますが、クリックで URL は開けません
...
| `openUrl`         | `"http://localhost:3000"` | 通知クリックで開く URL（terminal-notifier 使用時のみ有効）      |
...
これで node / gh / claude / terminal-notifier の絶対パスと PATH を解決した plist を `~/Library/LaunchAgents/com.github-pr-dashboard.notifier.plist` に生成します。続けて表示されるコマンドでロードします:
...
macOS の通知許可ダイアログが出たら許可してください（初回のみ）。
...
- 通知のクリックで URL を開くには `terminal-notifier` が必要です。osascript の `display notification` はクリックアクションを持てないため、terminal-notifier が無い場合はクリックしても `openUrl` を開けません
- 通知の送信元アプリ名は、terminal-notifier 使用時は terminal-notifier、osascript フォールバック時は実行プロセス（launchd 経由のため Node.js Foundation）になります
