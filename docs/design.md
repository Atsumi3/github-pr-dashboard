...
└── js/
    ...
    ├── sw.js           ServiceWorker (TTL 15min cache, network-first)
    ├── sw-register.js  scope:'/' で登録
    ├── setup.js        setup.html の PAT 入力ハンドラ
    ├── app.js          ダッシュボード本体
...
- 401 検知時は `caches.delete(API_CACHE)` のみ実行。SW はレスポンス本文を読めないため `LOGOUT_REQUIRED` の broadcast は意図的に行わず、PAT を消すかどうかは page-side `api.js` が `error.code === 'GITHUB_TOKEN_EXPIRED'` で判定する
- `CACHE_VERSION` (バンプすると activate で旧キャッシュを掃除)
...
- ローカル単独利用が前提
- token は localStorage に平文保存（XSS リスク許容、fine-grained PAT + 最小スコープを推奨）
- backend は `ALLOWED_ORIGINS` ベースの Origin / Referer ガードを実装
- token 切替検知時に backend の PR 一覧 / 詳細キャッシュと SW キャッシュを自動破棄
- AI 設定 API のシステムプロンプト更新は `X-Confirm-Ai-Config` ヘッダ必須
- ai-server は `AI_SHARED_SECRET` 必須、Host ヘッダ allowlist (DNS rebinding 緩和)、CLI whitelist で host 上の任意コマンド実行を防ぐ
- nginx CSP: `default-src 'self'`, `script-src 'self'` (unsafe-inline / unsafe-eval なし), `style-src 'self' https://fonts.googleapis.com 'unsafe-inline'` (動的 style のため `'unsafe-inline'` 残存), `font-src 'self' https://fonts.gstatic.com`, `img-src 'self' https://avatars.githubusercontent.com data:`, `connect-src 'self'`, `frame-ancestors 'none'`
- クライアント返却エラーは固定文言 + エラーコード。詳細は console.error のみ
